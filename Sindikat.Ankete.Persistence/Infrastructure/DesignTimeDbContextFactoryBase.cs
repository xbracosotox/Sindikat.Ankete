...
namespace Sindikat.Ankete.Persistence.Infrastructure
{
    public abstract class DesignTimeDbContextFactoryBase<T> : IDesignTimeDbContextFactory<T> where T : DbContext
    {
        private const string ConnectionStringName = "AnketeDatabase";
        private const string AspNetCoreEnvironment = "ASPNETCORE_ENVIRONMENT";

        public T CreateDbContext(string[] args)
        {
            var basePath = Directory.GetCurrentDirectory() + string.Format("{0}..{0}Sindikat.Ankete.API", Path.DirectorySeparatorChar);
